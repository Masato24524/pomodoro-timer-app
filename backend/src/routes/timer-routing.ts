...
router.get("/", async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.rpc("sum_task_times", {
      user_id: user?.id, // user.idを渡す
    }); //SQL Editorで作成した関数を呼び出す
...
router.get("/daily-time", async (req, res) => {
  ...
  try {
    ...
    const { data, error } = await supabase.rpc("sum_task_daily_times", {
      user_id: user?.id, // reqのuser.idを渡す
    ...
    const dataMap = new Map(
      data.map((item: dateType) => [item.task_date, item.total_hours])
    );
